...
Namelist /control/Max_words ,  Vector_Size, cosine_sample,  &
                ...
                test_out, test_matrix, run_num, group_out, NUM_ITER
!
! Start timing
!
Print*,"MINERVA utility version 0"
...
group_out     = blank; control_file = "control.txt"; 
...
! ************************************************************
    ...
    If(Do_Test_Phase) then
        ...
        do s=1,NUM_TEST
            ...
            elseif ((rank < 10) .or. (rank > 1000)) then
                notLastPerm = .false.
                perm = (/(i,i=1,MAX_WORDS)/)
                do i=1,indices(1)
                    call nexper(MAX_WORDS,perm,notLastPerm,even)
